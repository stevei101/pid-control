...
"""
   pid_list_stop.py A Python tool to control processes in Unix like systems by PID.
...
def get_pids():
    ...
    """
    try:
        pids = os.system('./list_ceilo_processes.sh')
    except OSError:
        logger.error('ERROR: Could not get list of PIDs.')
    with open ("ceilo.pid", 'rt') as pid_file:
        pid_list = []
